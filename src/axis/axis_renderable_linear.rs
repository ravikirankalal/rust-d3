...
impl super::axis_renderable::AxisRenderable for Axis<crate::scale::ScaleLinear> {
    fn render(&self, selection: &mut crate::selection::Selection) {
                ...
                match self.orientation {
                    AxisOrientation::Bottom | AxisOrientation::Top => {
                        selection.append("line")
                            ...
                            .attr("y1", "0")
                            .attr("y2", "-100%") // Example: full chart height
                            .attr("stroke", "#ccc")
                            .attr("class", "grid");
                    }
                    AxisOrientation::Left | AxisOrientation::Right => {
                        selection.append("line")
                            .attr("x1", "0")
                            .attr("x2", "100%") // Example: full chart width
        ...
        match self.orientation {
            ...
            AxisOrientation::Top => {
                println!("[AxisRenderable::ScaleLinear] Top axis ticks:");
                for tick in &ticks {
                    println!("  label: '{}' at position: {}", tick.label, tick.position);
                }
                if let (Some(first), Some(last)) = (ticks.first(), ticks.last()) {
            ...
            AxisOrientation::Right => {
                println!("[AxisRenderable::ScaleLinear] Right axis ticks:");
                for tick in &ticks {
                    println!("  label: '{}' at position: {}", tick.label, tick.position);
                }
                if let (Some(first), Some(last)) = (ticks.first(), ticks.last()) {
