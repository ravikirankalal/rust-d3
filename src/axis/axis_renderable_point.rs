...
impl<T: Clone + PartialEq + ToString> super::axis_renderable::AxisRenderable for Axis<crate::scale::ScalePoint<T>> {
    fn render(&self, selection: &mut crate::selection::Selection) {
        if let Some(hook) = &self.on_render { hook(); }
        ...
        if let Some(ref title) = self.title {
            match self.orientation {
                AxisOrientation::Bottom => {
                    ...
                    title_text.attr("x", "50%")
                        .attr("y", "40")
                        .attr("text-anchor", "middle")
                        .attr("class", "axis-title")
                        .text(title);
                    if let Some(style) = &self.title_style {
                        title_text.attr("font-family", &style.font)
                            .attr("fill", &style.color);
                        if let Some((x, y)) = style.position {
                            title_text.attr("x", &x.to_string())
                                .attr("y", &y.to_string());
                        }
                    }
                }
