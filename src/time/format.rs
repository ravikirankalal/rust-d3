...
use chrono::{NaiveDateTime, Datelike, Timelike, TimeZone, Utc, Local};
use crate::time::locale::TimeLocale;

/// Format a date/time using a specifier string and locale.
pub fn time_format_with_locale<Tz: TimeZone>(spec: &str, date: &NaiveDateTime, locale: &TimeLocale, _is_utc: bool) -> String {
        ...
        if c == '%' {
            ...
            if let Some(&next) = chars.peek() {
                ...
                out.push_str(&match code.as_str() {
                    ...
                    "%p" => if date.hour() < 12 { locale.am.to_string() } else { locale.pm.to_string() },
                    "%P" => if date.hour() < 12 { locale.am.to_lowercase() } else { locale.pm.to_lowercase() },
                    "%a" => {
                    ...
                    "%c" => time_format_with_locale::<Tz>("%a %b %e %H:%M:%S %Y", date, locale, _is_utc),
                    "%x" => time_format_with_locale::<Tz>("%m/%d/%y", date, locale, _is_utc),
                    "%X" => time_format_with_locale::<Tz>("%H:%M:%S", date, locale, _is_utc),
                    "%Z" => if _is_utc { "UTC".to_string() } else { Local::now().format("%Z").to_string() },
                    "%z" => if _is_utc { "+0000".to_string() } else { Local::now().format("%z").to_string() },
                    "%%" => "%".to_string(),
...
pub fn time_parse_with_locale(spec: &str, s: &str, locale: &TimeLocale, _is_utc: bool) -> Option<NaiveDateTime> {
    ...
    while i < spec_chars.len() {
        if spec_chars[i] == '%' {
            i += 1;
            if i >= spec_chars.len() { break; }
            let code = spec_chars[i];
            match code {
                'Y' => {
                    if idx + 4 > chars.len() { return None; }
                ...
                'L' => {
                    if idx + 3 > chars.len() { return None; }
                    millis = s[idx..idx+3].parse().ok();
                    idx += 3;
                },
                'f' => {
                    if idx + 6 > chars.len() { return None; }
                    millis = s[idx..idx+6].parse().ok().map(|v: u32| v / 1000);
                    idx += 6;
                },
                'p' | 'P' => {
                    if s[idx..].starts_with(locale.am) || s[idx..].starts_with(&locale.am.to_lowercase()) {
                        ampm = Some("AM");
                        idx += locale.am.len();
                    } else if s[idx..].starts_with(locale.pm) || s[idx..].starts_with(&locale.pm.to_lowercase()) {
                        ampm = Some("PM");
                        idx += locale.pm.len();
                    } else {
                        return None;
    ...
    NaiveDate::from_ymd_opt(y, m, d)
        .and_then(|date| date.and_hms_milli_opt(h, min, s, ms))
}
...
pub fn multi_format<'a, F: Fn(&NaiveDateTime) -> bool>(formats: Vec<(&'a str, F)>, date: &NaiveDateTime) -> String {
