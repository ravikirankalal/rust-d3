...
impl ScaleTime {
    ...
    pub fn scale(&self, x: chrono::NaiveDateTime) -> f64 {
        let t = (x.timestamp_millis() - self.domain[0].timestamp_millis()) as f64 /
    ...
    pub fn invert(&self, y: f64) -> chrono::NaiveDateTime {
        ...
        let millis = self.domain[0].timestamp_millis() as f64 + t * (self.domain[1].timestamp_millis() - self.domain[0].timestamp_millis()) as f64;
        chrono::NaiveDateTime::from_timestamp_opt((millis / 1000.0) as i64, ((millis % 1000.0) * 1_000_000.0) as u32).unwrap()
    }
}
