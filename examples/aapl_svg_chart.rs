...
fn main() {
    ...
    for (i, line) in reader.lines().enumerate() {
        ...
        closes.push(parts[4].parse::<f32>().unwrap_or(0.0));
    }
    let width = 600;
    let height = 300;
    let margin = 40;
    ...
    let area  = Area::new()
        .x(|d: &String, i: usize| margin as f64 + (i as f64) * ((width - 2 * margin) as f64) / ((n - 1) as f64))
        .y0(|d: &String,_| margin as f64 + (max_close - min_close) as f64 * ((height - 2 * margin) as f64) / (max_close - min_close) as f64)
        .y1(|d: &String, _| margin as f64 + (max_close as f64 - d as f64) as f64 * ((height - 2 * margin) as f64) / (max_close - min_close) as f64);
    
    svg.append("path")
      .attr("fill", "steelblue")
      .attr("d", area.generate(dates).as_str())
      .attr("stroke-width", "2");
