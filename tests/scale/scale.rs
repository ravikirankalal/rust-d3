...
fn test_log_scale() {
    ...
    assert!((scale.scale(1.0) - 0.0).abs() < 1e-8);
    assert!((scale.scale(10.0) - 1.0).abs() < 1e-8);
    assert!((scale.scale(100.0) - 2.0).abs() < 1e-8);
}

#[test]
fn test_pow_scale() {
    let scale = PowScale::new((0.0, 4.0), (0.0, 16.0), 2.0);
    assert_eq!(scale.scale(0.0), 0.0);
    assert_eq!(scale.scale(2.0), 4.0);
    assert_eq!(scale.scale(4.0), 16.0);
}
...
#[test]
fn test_band_scale() {
    let scale = BandScale::new(vec!["a", "b", "c"], (0.0, 90.0), 0.1);
